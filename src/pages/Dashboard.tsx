...
const tours = [
  {
    id: 1,
    name: "Первооткрытие Камчатки",
    description: "Вулканы, гейзеры и Тихий океан — незабываемое путешествие на край земли.",
    image: tourKamchatka,
  },
  {
    id: 2,
    name: "Все грани Байкала",
    description: "Глубочайшее озеро планеты: лёд, тайга и сибирское гостеприимство.",
    image: tourBaikal,
  },
  {
    id: 3,
    name: "Путешествие на Алтай",
    description: "Горные перевалы, бирюзовые реки и дикая природа Горного Алтая.",
    image: tourAltai,
  },
  {
    id: 4,
    name: "Сахалин и Итуруп",
    description: "Таинственные острова с вулканами, горячими источниками и океаном.",
    image: tourSakhalin,
  },
  {
    id: 5,
    name: "Южный Китай",
    description: "Карстовые горы, рисовые террасы и древние храмы юга Поднебесной.",
    image: tourChina,
  },
];
...
const Dashboard = () => {
  ...
  const [form, setForm] = useState({ clientName: "", clientContact: "", dates: "", comment: "" });

  useEffect(() => {
    const stored = localStorage.getItem("aventura_user");
    if (!stored) {
      navigate("/");
      return;
    }
    setUser(JSON.parse(stored));
    const apps = localStorage.getItem("aventura_apps");
    if (apps) setApplications(JSON.parse(apps));
  }, [navigate]);

  const handleLogout = () => {
    localStorage.removeItem("aventura_user");
    navigate("/");
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    localStorage.setItem("aventura_apps", JSON.stringify(updated));
    toast({ title: "Заявка отправлена!", description: `Тур: ${selectedTour}` });
    setModalOpen(false);
    setForm({ clientName: "", clientContact: "", dates: "", comment: "" });
  };
            ...
            <div className="space-y-2">
              {applications.map((app, i) => (
                <div key={i} className="bg-card rounded-xl p-4 border border-border flex items-center justify-between">
                  <div>
                    <p className="font-medium text-sm">{app.tourName}</p>
                    <p className="text-muted-foreground text-xs">Клиент: {app.clientName} · {app.date}</p>
                  </div>
            ...
            {tours.map((tour) => (
              <div key={tour.id} className="bg-card rounded-2xl border border-border overflow-hidden flex flex-col">
                ...
                <div className="p-4 flex flex-col flex-1 space-y-3">
                  ...
                  <Button
                    onClick={() => openModal(tour.name)}
                    className="w-full bg-primary text-primary-foreground hover:bg-gold-glow font-medium text-sm h-10"
                  >
                    Хочу продавать
                  </Button>
                </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="bg-card border-border text-foreground max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            />
            <Input
              placeholder="Контакт клиента (телефон/email)"
            ...
            <Textarea
              ...
              className="bg-secondary border-border min-h-[70px]"
            />
